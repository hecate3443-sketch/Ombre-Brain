...
3. 保留所有待办/未完成事项
4. 关键数字、日期、名称必须保留
5. 保留原文的叙述视角和人称——「我」不能替换为「作者」「用户」或其它第三人称词，「你」不能替换为「对方」「她」等。如果原文是第一人称，压缩后必须保持第一人称。
6. 目标压缩率 > 70%
...
DIGEST_PROMPT = """你是一个记忆拆分工具。用户会发送一段以「我」为第一人称的日常叙述，你的唯一任务是把这段文本按主题拆成多个独立条目。
...
- 把同一主题的零散段落合并到一条
- 删掉真正的重复内容（同一句话说了两遍）
- 删掉纯过渡废话（「然后呢」「接着」这类仅用于串联句子的词）

禁止的风格操作——以下任何一项都是错误：
- 换词——原文说「谁他妈祖上没有个短发老奶奶」，你不能写成「对此表示不以为然」
- 润色——原文说「叶子卷得东倒西歪像喝多了」，你不能写成「如同喝醉了一般」
- 改语气——粗口、口语、方言、比喻、情绪词全部原样保留
- 改人称——原文是「我」的第一人称，content 里也必须是「我」，不能变成「镜妤」「作者」「用户」或任何第三人称
- 总结复述——不要用你自己的话概括。原文怎么写的就怎么搬过去，只删不改

拆分规则：
1. 按主题/事件独立拆分，总条目 2~6 个
2. 同一主题的零散段落合并为一个条目
3. 太短的碎片（<30字）合并到最相关的条目
4. 待办事项单独提取为一条
5. 在 content 中对人名、地名、专有名词用 [[双链]] 标记（如 [[婷易]]、[[Obsidian]]），普通词汇不要加

输出格式（纯 JSON 数组，无其他内容）：
[
  {
    "name": "条目标题（10字以内）",
    "content": "拆分后的原文节选——逐字搬运，保留全部语气和用词",
    "domain": ["主题域1"],
    "valence": 0.7,
    "arousal": 0.4,
...
class Dehydrator:
    ...
    async def dehydrate(self, content: str, metadata: dict = None) -> str:
        ...
        # --- 内容已经很短，不需要压缩 ---
        if count_tokens_approx(content) < 250:
